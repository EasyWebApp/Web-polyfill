(() => {
  ...
  function isStructurallyValidLanguageTag(tag2) {
    try {
      parseUnicodeLanguageId(tag2.split(SEPARATOR));
    } catch (e) {
      return false;
    }
    return true;
  ...
  function getCalendarPreferenceDataForRegion(region) {
    ...
    return (calendars2[_region || ""] || calendars2["001"]).map(
      (c) => {
        if (c === "gregorian") {
          return "gregory";
        }
  ...
  function applyOptionsToTag(tag2, options) {
    ...
    invariant(
      isStructurallyValidLanguageTag(tag2),
    ...
    const languageId = parseUnicodeLanguageId(tag2);
    if (language !== void 0) {
    ...
    return Intl.getCanonicalLocales(
      emitUnicodeLocaleId(__spreadProps(__spreadValues({}, parseUnicodeLocaleId(tag2)), {
        lang: languageId
      }))
    )[0];
  }
  function applyUnicodeExtensionToTag(tag2, options, relevantExtensionKeys) {
    let unicodeExtension;
    let keywords = [];
    const ast = parseUnicodeLocaleId(tag2);
  ...
  function addLikelySubtags(tag2) {
    ...
    if (script && region) {
      const match2 = likelySubtags[emitUnicodeLanguageId({ lang, script, region, variants: [] })];
      if (match2) {
    ...
    if (script) {
      const match2 = likelySubtags[emitUnicodeLanguageId({ lang, script, variants: [] })];
      if (match2) {
        const parts2 = parseUnicodeLanguageId(match2);
        ast.lang = mergeUnicodeLanguageId(
          void 0,
    ...
    if (region) {
      const match2 = likelySubtags[emitUnicodeLanguageId({ lang, region, variants: [] })];
      if (match2) {
        const parts2 = parseUnicodeLanguageId(match2);
        ast.lang = mergeUnicodeLanguageId(
          void 0,
    ...
    const match = likelySubtags[lang] || likelySubtags[emitUnicodeLanguageId({ lang: "und", script, variants: [] })];
    if (!match) {
  ...
  function removeLikelySubtags(tag2) {
    ...
    maxLocale = emitUnicodeLanguageId(__spreadProps(__spreadValues({}, parseUnicodeLanguageId(maxLocale)), {
      variants: []
    }));
    const ast = parseUnicodeLocaleId(tag2);
    const {
    ...
    if (trial === maxLocale) {
      return emitUnicodeLocaleId(__spreadProps(__spreadValues({}, ast), {
