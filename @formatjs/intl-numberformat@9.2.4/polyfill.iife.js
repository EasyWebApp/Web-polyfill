(() => {
  ...
  function OrdinaryHasInstance(C, O, internalSlots) {
    ...
    if (internalSlots == null ? void 0 : internalSlots.boundTargetFunction) {
      let BC = internalSlots == null ? void 0 : internalSlots.boundTargetFunction;
  ...
  function CollapseNumberRange(numberFormat, result, { getInternalSlots: getInternalSlots2 }) {
    const internalSlots = getInternalSlots2(numberFormat);
  ...
  function CurrencyDigits(c, { currencyDigitsData: currencyDigitsData2 }) {
  ...
  function formatToParts(numberResult, data2, pl, options) {
    var _a;
    const { sign: sign2, exponent, magnitude } = numberResult;
    const { notation, style, numberingSystem } = options;
    const defaultNumberingSystem = data2.numbers.nu[0];
    ...
    if (style === "currency" && options.currencyDisplay !== "name") {
      const byCurrencyDisplay = data2.currencies[options.currency];
    ...
    if (!compactNumberPattern) {
      if (style === "decimal" || style === "unit" || style === "currency" && options.currencyDisplay === "name") {
        const decimalData = data2.numbers.decimal[numberingSystem] || data2.numbers.decimal[defaultNumberingSystem];
        numberPattern = getPatternForSign(decimalData.standard, sign2);
      } else if (style === "currency") {
        const currencyData = data2.numbers.currency[numberingSystem] || data2.numbers.currency[defaultNumberingSystem];
        numberPattern = getPatternForSign(currencyData[options.currencySign], sign2);
      } else {
        const percentPattern = data2.numbers.percent[numberingSystem] || data2.numbers.percent[defaultNumberingSystem];
    ...
    const numberParts = [];
    const symbols = data2.numbers.symbols[numberingSystem] || data2.numbers.symbols[defaultNumberingSystem];
      ...
      switch (part) {
        case "{0}": {
          numberParts.push(...partitionNumberIntoParts(
            ...
            !compactNumberPattern && ((_a = options.useGrouping) != null ? _a : true),
    ...
    switch (style) {
      case "currency": {
        if (options.currencyDisplay === "name") {
          const unitPattern = (data2.numbers.currency[numberingSystem] || data2.numbers.currency[defaultNumberingSystem]).unitPattern;
          let unitName;
          const currencyNameData = data2.currencies[options.currency];
          if (currencyNameData) {
      ...
      case "unit": {
        ...
        let unitData = data2.units.simple[unit];
        let unitPattern;
        if (unitData) {
          ...
          const numeratorUnitPattern = selectPlural(pl, numberResult.roundedNumber.times(getPowerOf10(exponent)).toNumber(), data2.units.simple[numeratorUnit][unitDisplay]);
          const perUnitPattern = data2.units.simple[denominatorUnit].perUnit[unitDisplay];
          if (perUnitPattern) {
            unitPattern = perUnitPattern.replace("{0}", numeratorUnitPattern);
          } else {
            const perPattern = data2.units.compound.per[unitDisplay];
            const denominatorPattern = selectPlural(pl, 1, data2.units.simple[denominatorUnit][unitDisplay]);
  ...
  function getCompactDisplayPattern(numberResult, pl, data2, style, compactDisplay, currencyDisplay, numberingSystem) {
    var _a, _b;
    const { roundedNumber, sign: sign2, magnitude } = numberResult;
    let magnitudeKey = String(__pow(10, magnitude));
    const defaultNumberingSystem = data2.numbers.nu[0];
    let pattern;
    if (style === "currency" && currencyDisplay !== "name") {
      const byNumberingSystem = data2.numbers.currency;
      const currencyData = byNumberingSystem[numberingSystem] || byNumberingSystem[defaultNumberingSystem];
      let compactPluralRules = (_a = currencyData.short) == null ? void 0 : _a[magnitudeKey];
      if (!compactPluralRules) {
        ...
        if (thresholds.length > 0 && magnitudeKey > thresholds[thresholds.length - 1]) {
          ...
          compactPluralRules = (_b = currencyData.short) == null ? void 0 : _b[magnitudeKey];
        }
      }
      if (!compactPluralRules) {
        return null;
      ...
      const byNumberingSystem = data2.numbers.decimal;
  ...
  function PartitionNumberRangePattern(numberFormat, x, y, { getInternalSlots: getInternalSlots2 }) {
  ...
  function FormatNumericRange(numberFormat, x, y, { getInternalSlots: getInternalSlots2 }) {
    const parts = PartitionNumberRangePattern(numberFormat, x, y, { getInternalSlots: getInternalSlots2 });
  ...
  function FormatNumericRangeToParts(numberFormat, x, y, { getInternalSlots: getInternalSlots2 }) {
    const parts = PartitionNumberRangePattern(numberFormat, x, y, { getInternalSlots: getInternalSlots2 });
...
Valid values are ${Array.from(VALID_ROUNDING_INCREMENTS).join(", ")}.`);
  ...
  function SetNumberFormatUnitOptions(internalSlots, options = /* @__PURE__ */ Object.create(null)) {
  ...
  function InitializeNumberFormat(nf, locales, opts, { getInternalSlots: getInternalSlots2, localeData, availableLocales, numberingSystemNames: numberingSystemNames2, getDefaultLocale, currencyDigitsData: currencyDigitsData2 }) {
    ...
    const internalSlots = getInternalSlots2(nf);
    ...
    if (style === "currency" && notation === "standard") {
      ...
      const cDigits = CurrencyDigits(currency, { currencyDigitsData: currencyDigitsData2 });
      mnfdDefault = cDigits;
      mxfdDefault = cDigits;
    } else {
      mnfdDefault = 0;
