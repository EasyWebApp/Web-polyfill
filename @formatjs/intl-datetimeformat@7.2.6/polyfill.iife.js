(() => {
  ...
  function ParseTimeZoneOffsetString(offsetString) {
    const match2 = OFFSET_TIMEZONE_FORMAT_REGEX.exec(offsetString);
    if (!match2) {
      return offsetString;
    }
    const sign2 = match2[1];
    const hours = match2[2];
    const minutes = match2[3] ? match2[3] : "00";
    const seconds = match2[4];
    const fractional = match2[5];
    let canonical = `${sign2}${hours}:${minutes}`;
  ...
  var OFFSET_TIMEZONE_PREFIX_REGEX2 = /^[+-]/;
  var OFFSET_TIMEZONE_FORMAT_REGEX2 = /^([+-])(\d{2})(?::?(\d{2}))?(?::?(\d{2}))?(?:\.(\d{1,9}))?$/;
  function IsTimeZoneOffsetString2(offsetString) {
    if (!OFFSET_TIMEZONE_PREFIX_REGEX2.test(offsetString)) {
      return false;
    }
    const match2 = OFFSET_TIMEZONE_FORMAT_REGEX2.exec(offsetString);
    if (!match2) {
      return false;
    }
    const hours = parseInt(match2[2], 10);
    const minutes = match2[3] ? parseInt(match2[3], 10) : 0;
    const seconds = match2[4] ? parseInt(match2[4], 10) : 0;
  ...
  var OFFSET_TIMEZONE_PREFIX_REGEX3 = /^[+-]/;
  var OFFSET_TIMEZONE_FORMAT_REGEX3 = /^([+-])(\d{2})(?::?(\d{2}))?(?::?(\d{2}))?(?:\.(\d{1,9}))?$/;
  function IsTimeZoneOffsetString3(offsetString) {
    return OFFSET_TIMEZONE_PREFIX_REGEX3.test(offsetString);
  }
  function ParseTimeZoneOffsetString2(offsetString) {
    const match2 = OFFSET_TIMEZONE_FORMAT_REGEX3.exec(offsetString);
    if (!match2) {
      return 0;
    }
    const sign2 = match2[1] === "+" ? 1 : -1;
    const hours = parseInt(match2[2], 10);
    const minutes = match2[3] ? parseInt(match2[3], 10) : 0;
    const seconds = match2[4] ? parseInt(match2[4], 10) : 0;
    const fractionalStr = match2[5] || "0";
    const paddedFractional = (fractionalStr + "000000000").slice(0, 9);
    const fractional = parseInt(paddedFractional, 10) / 1e6;
    const offsetMs = sign2 * (hours * 36e5 + minutes * 6e4 + seconds * 1e3 + fractional);
    return offsetMs;
  }
  function getApplicableZoneData(t, timeZone, tzData) {
    if (IsTimeZoneOffsetString3(timeZone)) {
      const offsetMs = ParseTimeZoneOffsetString2(timeZone);
  ...
  function FormatDateTimePattern(dtf, patternParts, x, {
    ...
    const hasOtherDateFields = patternParts.some(
      (part) => part.type === "day" || part.type === "year" || part.type === "weekday" || part.type === "era"
    );
  ...
  function PartitionDateTimeRangePattern(dtf, x, y, implDetails) {
    ...
    const { getInternalSlots: getInternalSlots2, tzData, localeData } = implDetails;
    const internalSlots = getInternalSlots2(dtf);
    const dataLocale = internalSlots.dataLocale;
    const dataLocaleData = localeData[dataLocale];
  ...
  // packages/intl-datetimeformat/src/data/links.generated.ts
  var links_generated_default = {
  ...
  var UPPERCASED_LINKS = Object.keys(links_generated_default).reduce(
    (all, l) => {
      all[l.toUpperCase()] = links_generated_default[l];
      return all;
    },
    {}
  );
  ...
  function findIntervalFormat(skeleton, intervalFormats) {
    ...
    const canonical = timePart.replace(/[^a-zA-Z]/g, "").replace(/[abB]/g, "").replace(/(.)\1+/g, "$1");
    if (canonical === timePart)
      return void 0;
    const canonicalKey = `${datePart}, ${canonical}`;
    return intervalFormats[canonicalKey];
  }
  DateTimeFormat.__addLocaleData = function __addLocaleData(...data2) {
      ...
      for (const calendar in formats) {
        processedData.formats[calendar] = Object.keys(formats[calendar]).map(
          (skeleton) => parseDateTimeSkeleton(
            ...
            intervalFormats[skeleton] || findIntervalFormat(skeleton, intervalFormats),
